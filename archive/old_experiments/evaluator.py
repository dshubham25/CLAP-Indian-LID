...
def precompute_embeddings(model, processor):
    embeddings = {}
    ...
    with torch.no_grad():
        ...
        embeddings["mean_noise_embed"] = torch.mean(noise_embeds, dim=0, keepdim=True)
        
    lang_embeddings = {}
    for language, prompts in EKSTEP_PROMPTS.items():
        lang_embeddings[language] = get_averaged_text_embeddings(model, processor, prompts)
        
    embeddings["lang_embeddings"] = lang_embeddings
    return embeddings

def predict_language(audio_path, model, processor, precomputed_embeddings=None):
    ...
    with torch.no_grad():
        ...
        if precomputed_embeddings is not None:
            mean_noise_embed = precomputed_embeddings["mean_noise_embed"]
        else:
    ...
    if precomputed_embeddings is not None:
        lang_embeddings = precomputed_embeddings["lang_embeddings"]
        for language, text_embed in lang_embeddings.items():
